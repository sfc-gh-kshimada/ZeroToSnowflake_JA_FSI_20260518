...
-- 内部ステージ (セクション 2(a) でファイルアップロード→ COPY INTO)
CREATE OR REPLACE STAGE fsi_zts_101.raw_trade.csv_stage
COMMENT = 'CSV ファイル取り込み用の内部ステージ (assets/sample_data/trade_csv/ をアップロード)'
FILE_FORMAT = fsi_zts_101.public.csv_ff;

CREATE OR REPLACE STAGE fsi_zts_101.raw_trade.json_stage
COMMENT = 'JSON ファイル取り込み用の内部ステージ (assets/sample_data/customer_json/ をアップロード)'
FILE_FORMAT = fsi_zts_101.public.json_ff;

CREATE OR REPLACE STAGE fsi_zts_101.raw_trade.xml_stage
COMMENT = 'XML (SWIFT MX 風) ファイル取り込み用の内部ステージ (assets/sample_data/swift_xml/ をアップロード)'
...
(
    ...
    customer_name      VARCHAR(200),       -- マスキング対象 (PII)
...
-- 顧客マスタ (50,000 社の架空企業 — バリエーション豊富な法人名)
-- 日本語社名 100 パターン + 英語社名 100 パターン = 計 200 社名ベースから重複排除しつつ生成
...
SELECT
    ...
    -- 200 パターンの架空法人名 (日本語 + 英語)
    DECODE(MOD(SEQ4(), 200),
        -- 日本語 100 社 (ギリシャ文字 + 業種 のパターン)
        ...
        99, '日本ブリッジテクノ',
        -- 英語 100 社
        100, 'Alpha Trading Co., Ltd.',     101, 'Beta Manufacturing Inc.',
        102, 'Gamma Holdings International', 103, 'Delta Electronics Corp.',
        104, 'Epsilon Logistics Pte. Ltd.', 105, 'Zeta Finance Corporation',
        106, 'Eta Precision Industries',     107, 'Theta Technologies LLC',
        108, 'Iota Capital Partners',        109, 'Kappa Solutions Group',
        110, 'Lambda Energy Systems',        111, 'Mu Shipping Ltd.',
        112, 'Nu Pharmaceuticals Inc.',      113, 'Xi Heavy Industries',
        114, 'Omicron Foods Corp.',          115, 'Pi Communications AG',
        116, 'Rho Real Estate GmbH',        117, 'Sigma Construction Ltd.',
        118, 'Tau Medical Devices',          119, 'Upsilon Securities PLC',
        120, 'Phi Insurance Brokers',        121, 'Chi Asset Management',
        122, 'Psi Information Systems',      123, 'Omega Partners LP',
        124, 'Pacific Rim Trading',          125, 'Asia Gateway Corp.',
        126, 'Global Bridge Financial',      127, 'Trans-Ocean Logistics',
        128, 'Northern Star Resources',      129, 'Southern Cross Ventures',
        130, 'Eastern Promise Capital',      131, 'Western Digital Holdings',
        132, 'Sunrise Technology Inc.',      133, 'Moonlight Industries',
        134, 'Starfield Enterprises',        135, 'Cloudbreak Solutions',
        136, 'Riverstone Capital',           137, 'Mountain Peak Mining',
        138, 'Ocean Deep Exploration',       139, 'Skyline Infrastructure',
        140, 'Greenfield Agriculture',       141, 'Ironclad Manufacturing',
        142, 'Silverline Transport',         143, 'Golden Gate Investments',
        144, 'Crystal Clear Water',          145, 'Diamond Edge Tech',
        146, 'Sapphire Analytics',           147, 'Emerald City Consulting',
        148, 'Ruby Networks PLC',            149, 'Pearl Harbor Shipping',
        150, 'Coral Reef Biotech',           151, 'Titanium Aerospace',
        152, 'Platinum Financial Group',     153, 'Bronze Age Manufacturing',
        154, 'Carbon Zero Energy',           155, 'Quantum Leap Computing',
        156, 'Fusion Power Systems',         157, 'Nexus Point Capital',
        158, 'Apex Global Trading',          159, 'Zenith Pharmaceutical',
        160, 'Summit Peak Resources',        161, 'Vertex Data Solutions',
        162, 'Horizon Maritime Ltd.',        163, 'Eclipse Digital Media',
        164, 'Aurora Borealis Energy',       165, 'Cascade Hydro Power',
        166, 'Tidal Wave Logistics',         167, 'Firefly Semiconductors',
        168, 'Thunderbolt Electric',         169, 'Whirlwind Aviation',
        170, 'Serenity Healthcare',          171, 'Momentum Sports Tech',
        172, 'Harmony Music Group',          173, 'Velocity Motors Corp.',
        174, 'Clarity Vision Systems',       175, 'Integrity Audit Services',
        176, 'Pinnacle Wealth Management',   177, 'Fortitude Insurance',
        178, 'Resolve Legal Services',       179, 'Prudence Investment Trust',
        180, 'Liberty Trade Finance',        181, 'Heritage Property Fund',
        182, 'Pioneer Agri-Tech',            183, 'Frontier Mining Co.',
        184, 'Vanguard Defence Systems',     185, 'Guardian Security Corp.',
        186, 'Sentinel Cyber Defence',       187, 'Shield Data Protection',
        188, 'Bastion Risk Management',      189, 'Citadel Capital Markets',
        190, 'Fortress Asset Holdings',      191, 'Rampart Engineering',
        192, 'Bulwark Construction',         193, 'Aegis Consulting Group',
        194, 'Paladin Investment Bank',      195, 'Spartan Logistics',
        196, 'Viking Offshore',              197, 'Samurai Fund Management',
        198, 'Shogun Trading House',         199, 'Ronin Advisory') AS customer_name,
...
SELECT
    ...
    -- 法人顧客名 — customers テーブルと同じ 200 パターンから選出 (先頭 100 社 = 日本語)
...
/*--
 8. Git リポジトリ統合 (Workspaces + データファイル参照)
    ────────────────────────────────────────────────────────────────
    GitHub 上の公開リポジトリを Snowflake に同期し、
    リポジトリ内の Excel / XML / CSV / JSON ファイルを
    直接参照してデータロードします (Stage への手動アップロード不要)。

    Workspaces の「From Git repository」で同リポジトリを接続すると、
    SQL ファイルの編集・実行も Snowsight 上で完結します。
--*/

-- 8.1 API Integration (GitHub HTTPS 用)
-- 既に存在する場合はスキップされます (OR REPLACE で冪等)
CREATE OR REPLACE API INTEGRATION git_api_integration
    API_PROVIDER = git_https_api
    API_ALLOWED_PREFIXES = ('https://github.com/sfc-gh-kshimada/')
    ENABLED = TRUE;

-- 8.2 Git Repository オブジェクト作成
-- 公開リポジトリのため認証不要 (GIT_CREDENTIALS は省略)
...
SELECT '✓ FSI Zero To Snowflake セットアップが完了しました。' AS status,
       'fsi_zts_101 データベース・スキーマ・ロール・合成データ・Git リポジトリが利用可能です。' AS message,
       '次のステップ: Snowsight Workspaces で「From Git repository」から ' ||
       'https://github.com/sfc-gh-kshimada/ZeroToSnowflake_JA_FSI_20260518.git を接続し、' ||
       'scripts/ 配下の SQL を順次実行してください。' ||
       'データファイルは @fsi_zts_101.public.fsi_zts_repo/branches/main/assets/ から直接参照できます。' AS next_step;
