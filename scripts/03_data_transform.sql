...
/******************************************************************************
 ...
 ██║     ██║  ██║██║  ██║   ██║       ██║  ██║
 ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝       ╚═╝  ╚═╝
  Tasks + Streams  -- 増分処理 (オンプレバッチの代替)   ~20 分
 ******************************************************************************/
...
/*----------------------------------------------------------------------------------
 1. Stream の作成
    ...
    オンプレ環境では「前回処理済みのタイムスタンプ以降のレコードを WHERE で抽出」
    という方法が一般的ですが、Stream を使えばそのロジックが不要になります。
    Stream は "消費" ベースで動作し、一度 DML で読み取ると自動的に進みます。
...
-- Snowsight の UI で Data > Databases > fsi_zts_101 > HARMONIZED > Tasks からも
-- 実行履歴を確認できます。
...
/*----------------------------------------------------------------------------------
 8. DAG 可視化 (Snowsight 手順)
    ...
    手順:
      1. Snowsight の左メニューで [Data] をクリック
      2. [Databases] を展開し、fsi_zts_101 を選択
      3. HARMONIZED スキーマを展開 → [Dynamic Tables] を展開
...
/*----------------------------------------------------------------------------------
 9. 法人営業データの分析クエリ (Dynamic Table の結果を活用)
    ...
    これらのクエリは Streamlit ダッシュボードや BI ツール (Tableau 等) の
...
/*----------------------------------------------------------------------------------
 10. Task + Stream vs Dynamic Table: 比較と推奨
    ...
    ┌──────────────────────────────────────────────────────────────────────┐
    │   オンプレバッチサーバ → Snowflake 移行のメリット                    │
    ├──────────────────────────────────────────────────────────────────────┤
    │                                                                      │
    │  [オンプレバッチ処理の課題]                                          │
    │   - 夜間バッチの運用負荷 (cron 管理、監視、障害対応)                │
    │   - サーバのサイジング・パッチ適用                                    │
    │   - バッチ失敗時の手動リカバリ                                        │
    │   - スケールアウトが困難 (ハードウェア増設)                          │
    │   - 依存関係の管理が複雑 (シェルスクリプトの連鎖)                    │
    │                                                                      │
    ...
    次のセクション:
      - Section 4: データガバナンス (マスキング・行アクセスポリシー・タグ)
----------------------------------------------------------------------------------*/
