...
AS (
    SELECT
        ...
        amount
    FROM raw_trade.trade_transactions
    WHERE trade_date >= DATEADD(month, -2, CURRENT_DATE())
);
...
/*----------------------------------------------------------------------------------
 6. Semantic View の作成 (Cortex Analyst 用)
    -------------------------------------------------
    ...
    アーキテクチャ:
      Semantic View (SQL DDL)
        ├── TABLES: 対象テーブル/ビューの定義 (SYNONYMS 付き)
        ├── FACTS: 集計対象の数値カラム
        ├── DIMENSIONS: フィルタ・グループ化に使う属性カラム
        ├── METRICS: よく使う集計の定義 (SUM / COUNT / AVG ...)
        └── AI_SQL_GENERATION: SQL 生成時のルール
                 ↓
...
-- =====================================================================
-- [Option] Cortex Agent + Snowflake Intelligence
-- =====================================================================

/*----------------------------------------------------------------------------------
 8. [Option] Cortex Agent — BI エージェントの構築
    -------------------------------------------------
...
/*----------------------------------------------------------------------------------
 9. [Option] Snowflake Intelligence — 会話型 BI インターフェース
    -------------------------------------------------
...
-- Cortex Agent を呼び出すために必要なデータベースロールを付与
USE ROLE accountadmin;
...
/*--
    ...
    1. Snowsight → [AIとML] → [Snowflake Intelligence]
